...
const metrics = [
  ...
  { title: "Blood Pressure", value: "120/80", unit: "mmHg", status: "optimal", icon: Activity, color: "text-success" },
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* Risk Overview */}
      <motion.div {...fadeUp} transition={{ delay: 0 }} className="glass-card p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-muted-foreground mb-1">Hypoglycemia Risk Score</p>
            <div className="flex items-baseline gap-3">
              <span className={`text-5xl font-semibold ${riskColor} glow-text-primary`}>{riskScore}</span>
          ...
          <div className="hidden md:block h-24 w-24 rounded-full border-4 border-primary/20 flex items-center justify-center relative">
            <svg className="h-24 w-24 -rotate-90" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="42" fill="none" stroke="hsl(var(--muted))" strokeWidth="6" />
              <circle cx="50" cy="50" r="42" fill="none" stroke="hsl(var(--primary))" strokeWidth="6" strokeDasharray={`${riskScore * 2.64} 264`} strokeLinecap="round" className="transition-all duration-1000" />
            </svg>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {metrics.map((m, i) => (
          <motion.div key={m.title} {...fadeUp} transition={{ delay: 0.1 + i * 0.05 }} className="glass-card-hover p-5">
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              />
              <Area type="monotone" dataKey="glucose" stroke="hsl(187, 100%, 50%)" fill="url(#glucoseGrad)" strokeWidth={2} />
              <Area type="monotone" dataKey="risk" stroke="hsl(0, 100%, 65%)" fill="url(#riskGrad)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </motion.div>
      ...
      <motion.div {...fadeUp} transition={{ delay: 0.4 }}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {quickActions.map((a) => (
            <Link key={a.label} to={a.path} className="glass-card-hover p-4 flex flex-col items-center gap-3 text-center cursor-pointer">
